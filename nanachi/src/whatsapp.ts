...
import makeWASocket, { DisconnectReason, type WASocket, type GroupMetadata, type WAMessage } from "baileys";
import pino from "pino";

import { createAuthState } from "./auth";
import * as ipc from "./ipc";
import type { ContactData, GroupData, MessageData, ParticipantData } from "./types";
...
export async function startAccount(accountId: string): Promise<void> {
    ...
    const { state, saveCreds, getSerializedState } = await createAuthState(accountId, savedAuthState);
    ...
    sock.ev.on("connection.update", (update) => {
        ...
        if (connection === "close") {
            ...
            if (statusCode === DisconnectReason.loggedOut) {
                ...
                ipc.sendDisconnected(accountId, lastDisconnect?.error?.message || "Unknown");
                if (shouldReconnect) {
...
export async function sendMessage(accountId: string, to: string, content: string): Promise<boolean> {
    const conn = connections.get(accountId);
    if (!conn) {
