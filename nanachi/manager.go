...
// O sqlstore.Container expõe o *sql.DB internamente; replicamos a tabela aqui
// num conn separado para não interferir com o pool dele.
...
// resolveDevice escolhe (ou cria) o *store.Device associado ao account_id.
// Se já há um JID salvo, recupera o device existente; senão, cria novo (que
// disparará pareamento via QR no whatsmeow).
...
// reconcileAccount força um re-emit de tudo que o whatsmeow já sabe sobre
// contatos, grupos e newsletters. Útil para reconstruir o tina.db a partir
// do whatsmeow.db sem precisar de re-pareamento.
...
func (m *Manager) sendMessage(accountID, to, content string) (bool, error) {
	m.mu.Lock()
	client := m.clients[accountID]
	m.mu.Unlock()
	if client == nil {
